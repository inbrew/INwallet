...
// api
import { getTransaction } from "../../api/web3";
...
export default function TransactionListItem() {
  ...
  const handleEventTransaction = useCallback(async () => {
    if (transactions.maticTx) {
      setLoading({
        isLoading: true,
      });

      const resultGetTx = await getTransaction(transactions.maticTx);
  ...
  }, [handleEventTransaction, transactions.maticTx]);
          ...
          <Dialog open={open} onClose={handleClose}>
            ...
            <List sx={{ pt: 0 }}>
              ...
              >
                ...
                {modalData ? modalData.gasPrice : null} MATIC(Mumbai)
                <Typography variant="h6" sx={{ mt: "10px" }}>
                  거래 가격(Value)
                </Typography>
                {modalData ? modalData.value : null} MATIC(Mumbai)
              </ListItem>
            </List>
          </Dialog>
        </Box>
